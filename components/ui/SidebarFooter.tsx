...
export default function SidebarFooter({ collapsed }: SidebarFooterProps) {
  ...
  if (collapsed) {
    return (
      <div className={`p-2 border-t ${isDark ? 'border-white/10' : 'border-black/10'}`}>
        ...
        {!isLoading && !user && (
          <Link href="/login" className={`block w-full p-2 mt-1 rounded-lg text-center transition-colors ${isDark ? 'hover:bg-white/10 text-zinc-400' : 'hover:bg-black/10 text-zinc-600'}`}>
            <svg className="w-4 h-4 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
          </Link>
        )}
      </div>
    );
  ...
  return (
    <div className={`p-3 border-t ${isDark ? 'border-white/10' : 'border-black/10'}`}>
      {!isLoading && user && (
        <div className={`flex items-center gap-2 mb-2 px-1 py-1.5 rounded-lg ${isDark ? 'text-zinc-300' : 'text-zinc-700'}`}>
      ...
      {!isLoading && user && (
        <div className="flex items-center gap-1 mb-2">
          ...
          <button onClick={signOut} className={`px-2 py-1 rounded text-[11px] transition-colors ${isDark ? 'hover:bg-white/10 text-zinc-500 hover:text-zinc-300' : 'hover:bg-black/10 text-zinc-500 hover:text-zinc-700'}`}>
            Ulos
          </button>
        </div>
      )}
      {!isLoading && !user && (
        <Link href="/login" className={`block text-center px-2 py-1.5 mb-2 rounded-lg text-xs transition-colors ${isDark ? 'bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-zinc-200' : 'bg-black/5 hover:bg-black/10 text-zinc-600 hover:text-zinc-900'}`}>
          Kirjaudu sisään
        </Link>
      )}
