...
export default function UnemploymentSettings() {
  const { unemployment, theme, setUnemploymentYear, setUnemploymentDisplayMode } = useUnifiedFilters();
    ...
    <div className="space-y-3">
      ...
      <div>
        ...
        <div className={`grid grid-cols-2 gap-1 p-1 rounded-lg mt-1 ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
          <button
            onClick={() => setUnemploymentDisplayMode('perCapita')}
            className={`px-2 py-1.5 text-xs font-medium rounded-md transition-all ${
              unemployment.displayMode === 'perCapita'
          ...
          <button
            onClick={() => setUnemploymentDisplayMode('absolute')}
            className={`px-2 py-1.5 text-xs font-medium rounded-md transition-all ${
              unemployment.displayMode === 'absolute'
                ? 'bg-blue-600 text-white shadow-sm'
                : isDark ? 'text-zinc-400 hover:text-zinc-300' : 'text-zinc-600 hover:text-zinc-900'
            }`}
          >
      ...
      <div className={`border-t pt-2 ${isDark ? 'border-white/5' : 'border-black/5'}`}>
        ...
        {infoExpanded && (
          <div className={`mt-2 p-2.5 rounded-lg text-xs leading-relaxed space-y-1.5 ${
            ...
            <p><span className="font-semibold">Lähde:</span> Tilastokeskus &ndash; Työnvälitystilasto</p>
            <p><span className="font-semibold">Taulukko:</span> 12ti &ndash; Työttömät työnhakijat kunnittain</p>
            <p><span className="font-semibold">Työttömyysaste:</span> työttömät / työvoima &times; 100</p>
          </div>
        )}
      </div>
    </div>
