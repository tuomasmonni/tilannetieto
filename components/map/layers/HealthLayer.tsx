...
const SOURCE_ID = 'health-municipalities';
const FILL_LAYER = 'health-fill';
const OUTLINE_LAYER = 'health-outline';
...
export function HealthLayer({ map }: Props) {
  const { health, setHealthLoading } = useUnifiedFilters();
  const { indicator, year, layerVisible } = health;
  ...
  const indicatorMeta = SOTKANET_INDICATORS.find(i => i.id === indicator);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const res = await fetch(`/api/health?indicator=${debouncedIndicator}&year=${debouncedYear}`);
    ...
    loadData();
  }, [debouncedIndicator, debouncedYear, layerVisible, setHealthLoading]);
    ...
    const addLayer = () => {
      try {
        ...
        map.addLayer({
          ...
          source: SOURCE_ID,
          paint: {
            'line-color': ['case', ['boolean', ['feature-state', 'hover'], false], '#1f2937', '#d1d5db'],
        ...
        console.error('Failed to add health layer:', err);
      }
    };
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: any) => {
      ...
      const displayValue = value != null ? `${Number(value).toLocaleString('fi-FI')} ${unit}` : 'Ei dataa';
      ...
      const html = `
        <div class="backdrop-blur-md bg-white/95 dark:bg-gray-900/95 text-gray-900 dark:text-gray-100 px-3 py-2 rounded-xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 min-w-[180px]">
          ...
          <div class="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">${indicatorMeta?.label || ''} &middot; ${props?.year || year}</div>
          <div class="text-[10px] text-gray-500 dark:text-gray-400">Lähde: THL / Sotkanet</div>
        </div>
      `;
  ...
  }, [map, loaded, indicator, year, indicatorMeta]);
