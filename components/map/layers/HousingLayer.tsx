...
import type { HousingMapGeoJSON } from '@/lib/data/housing/api';
...
const SOURCE_ID = 'housing-municipalities';
const FILL_LAYER = 'housing-fill';
const OUTLINE_LAYER = 'housing-outline';
...
export function HousingLayer({ map }: Props) {
  const { housing, setHousingLoading } = useUnifiedFilters();
  const { year, layerVisible } = housing;
  const [loaded, setLoaded] = useState(false);
  const [data, setData] = useState<HousingMapGeoJSON | null>(null);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const res = await fetch(`/api/housing?year=${debouncedYear}`);
    ...
    loadData();
  }, [debouncedYear, layerVisible, setHousingLoading]);
  ...
  useEffect(() => {
    ...
    const addLayer = () => {
      try {
        ...
        console.error('Failed to add housing layer:', err);
      }
    };
      ...
      const html = `
        <div class="backdrop-blur-md bg-white/95 dark:bg-gray-900/95 text-gray-900 dark:text-gray-100 px-3 py-2 rounded-xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 min-w-[180px]">
          ...
          <div class="text-xs text-gray-600 dark:text-gray-300 mt-1">
            <span class="font-medium text-gray-900 dark:text-gray-100">${(props?.pricePerSqm || 0).toLocaleString('fi-FI')} €/m²</span>
          </div>
